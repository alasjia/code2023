...
#-------------数据读取及处理
#读取原始文件 并将日期设置为索引
data=pd.read_csv(r"E:\BJTU\其他\时间序列数据分析与挖掘\CNN实验数据\alcohol.csv",index_col="DATE",na_values="+9999,9")
#选取酒精销售量（S4248SM144NCEN）一列
data=data["S4248SM144NCEN"]
#将data.index设置为时间格式
data.index=pd.to_datetime(data.index)
#设置开始及阶数时间 选取一段数据
start_time=pd.to_datetime("1998-01-01")
end_time=pd.to_datetime("2005-01-01")
data=data[start_time:end_time]
#从data中剔除TMP列为NaN的数据
data=data.dropna()
#将时间补全（前面有丢弃操作）并将间隔设置为1天 重新设置data的索引
...
lr=0.001#学习率
'''
通过尝试调整多个超参数，发现调整batch_size对于收敛速度提升效果最为显著，具有越小收敛速度越快的趋势
但是过小的batch_size可能会导致训练速度较慢，因此均衡考虑采用batch_size=32
增加训练轮数，提升收敛效果
'''
num_epochs=300#训练轮数
